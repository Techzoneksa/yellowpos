...
import { listRecentOrders, getOrder } from "@/lib/api-client";
...
export function RecentOrdersScreen() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data: any = await listRecentOrders({ q: q.trim() || undefined, today: todayOnly, shiftOnly, limit: 100, offset: 0 });
      setRows(data || []);
    } catch (e: any) {
  ...
  const reprint = async (orderId: string) => {
    setBusyId(orderId);
    try {
      const d: any = await getOrder({ order_id: orderId });
      const o = d.order;
