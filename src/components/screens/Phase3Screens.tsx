...
export function ManagerSuppliers() {
  ...
  const reload = async () => {
    setLoading(true);
    try { const r: any = await listSuppliers(); setSuppliers(r.suppliers); }
    ...
    <ManagerLayout>
                ...
                <td className="p-3">
                  <div className="flex justify-end gap-1">
                    ...
                    <Button size="sm" variant="ghost"
                      onClick={async () => {
                        try { await setSupplierActive({ id: s.id, active: !s.active }); toast.success("✓"); reload(); }
                        catch (e: any) { toast.error(e.message); }
                      }}>
      ...
      {(creating || editing) && (
        <SupplierDialog
          ...
          onSave={async (data) => {
            try {
              await upsertSupplier({ ...data, id: editing?.id } as any);
...
function SupplierProfileSheet({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  useEffect(() => {
    (async () => {
      try { setData(await getSupplierProfile({ id })); } catch (e: any) { toast.error(e.message); }
      finally { setLoading(false); }
    })();
  }, [id]);
  return (
...
export function ManagerPurchases() {
  ...
  const reload = async () => {
    setLoading(true);
    try { const r: any = await listPurchases(); setRows(r.purchases); }
    catch (e: any) { toast.error(e.message); }
    finally { setLoading(false); }
  };
  useEffect(() => { reload(); }, []);
...
function PurchaseDialog({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [s, i]: any = await Promise.all([listSuppliers(), listInventory()]);
...
function PurchaseViewSheet({ id, onClose }: { id: string; onClose: () => void }) {
  const { lang, fmtMoney } = useApp();
  const [data, setData] = useState<any>(null);
  useEffect(() => { (async () => { try { setData(await getPurchase({ id })); } catch (e: any) { toast.error(e.message); } })(); }, [id]);
  return (
    <Sheet open onOpenChange={(o) => !o && onClose()}>
      <SheetContent side={lang === "ar" ? "left" : "right"} className="w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader>
...
export function ManagerInventory() {
  ...
  const reload = async () => {
    setLoading(true);
    try { const r: any = await listInventory(); setItems(r.items); }
    catch (e: any) { toast.error(e.message); }
    finally { setLoading(false); }
  };
  useEffect(() => { reload(); }, []);
              ...
              <tr key={it.id} className={cn("border-t", !it.active && "opacity-50")}>
                ...
                <td className="p-3">
                  <div className="flex justify-end gap-1">
                    ...
                    <Button size="sm" variant="ghost"
                      onClick={async () => {
                        try { await setInventoryActive({ id: it.id, active: !it.active }); toast.success("✓"); reload(); }
                        catch (e: any) { toast.error(e.message); }
                      }}>
                      <X className="h-3.5 w-3.5 text-destructive" />
                    </Button>
      ...
      <DialogContent className="max-w-xl">
        ...
        <DialogFooter>
          ...
          <Button disabled={!f.name_ar || saving}
            onClick={async () => {
              setSaving(true);
              try { await upsertInventoryItem({ ...f, id: item?.id } as any); toast.success(lang === "ar" ? "تم الحفظ" : "Saved"); onSaved(); }
              catch (e: any) { toast.error(e.message); }
              finally { setSaving(false); }
            }}>
            {saving && <Loader2 className="me-1 h-3 w-3 animate-spin" />}
...
function MovementsSheet({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  useEffect(() => {
    (async () => {
      try { const r: any = await listItemMovements({ item_id: id, limit: 100 }); setRows(r.movements); }
      catch (e: any) { toast.error(e.message); }
      finally { setLoading(false); }
    })();
  }, [id]);
...
export function ManagerRecipes() {
  ...
  const reload = async () => {
    setLoading(true);
    try {
      const [i, r]: any = await Promise.all([listInventory(), listRecipes()]);
    ...
    <ManagerLayout>
                ...
                <tr key={p.id} className="border-t">
                  ...
                  <td className="p-3 text-end">
                    ...
                    {r && <Button size="sm" variant="ghost" onClick={async () => {
                      ...
                      try { await deleteRecipe({ id: p.id }); toast.success("✓"); reload(); }
                      catch (e: any) { toast.error(e.message); }
      ...
      {editing && editingProduct && (
        <RecipeDialog
          ...
          onSaved={async (ings) => {
            try {
              await saveRecipe({ product_id: editingProduct.id, active: true, ingredients: ings });
...
export function ManagerAdjustments() {
  ...
  const reload = async () => {
    setLoading(true);
    try {
      const [a, i]: any = await Promise.all([listAdjustments(), listInventory()]);
    ...
    <ManagerLayout>
      ...
      {open && (
        <AdjustmentDialog inventory={inventory} onClose={() => setOpen(false)}
          onSaved={async (data) => { try { await createAdjustment(data); toast.success(lang === "ar" ? "تمت التسوية" : "Adjusted"); setOpen(false); reload(); } catch (e: any) { toast.error(e.message); } }} />
      )}
    </ManagerLayout>
  );
}
...
export function ManagerWaste() {
  ...
  const reload = async () => {
    setLoading(true);
    try { const [w, i]: any = await Promise.all([listWaste(), listInventory()]); setRows(w.records); setInventory(i.items.filter((x: any) => x.active)); }
    catch (e: any) { toast.error(e.message); }
    finally { setLoading(false); }
  };
  useEffect(() => { reload(); }, []);
    ...
    <ManagerLayout>
      ...
      {open && (
        <WasteDialog inventory={inventory} onClose={() => setOpen(false)}
          onSaved={async (data) => { try { await createWaste(data); toast.success(lang === "ar" ? "تم التسجيل" : "Recorded"); setOpen(false); reload(); } catch (e: any) { toast.error(e.message); } }} />
      )}
    </ManagerLayout>
  );
}
