...
import { closeShift as closeShiftApi, getShiftSummary } from "@/lib/api-client";
...
export function CloseShiftScreen() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const res = await getShiftSummary({ shift_id: shift.id }) as ShiftStats;
        if (cancelled) return;
        setStats(res);
  ...
  const submitCloseShift = async () => {
    ...
    setSubmitting(true);
    try {
      await closeShiftApi({ shift_id: shift.id, closing_cash: Number(actual) || 0 });
