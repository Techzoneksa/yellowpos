...
import { recordCashMovement, listCashMovements } from "@/lib/api-client";
...
export function CashDrawerDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const refresh = async () => {
    ...
    setLoadingList(true);
    try {
      const rows: any[] = await listCashMovements({ shift_id: shift.id });
      setHistory(rows);
    } catch (e: any) {
      // silent
    } finally {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      await recordCashMovement({ type, amount: a, reason });
