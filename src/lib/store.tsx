...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const refreshHeld = async (shiftId?: string) => {
    setHeldLoading(true);
    try {
      const rows: any[] = await api.listHeldOrders();
  ...
  useEffect(() => {
    const hydrateShift = async (role: UserRole): Promise<boolean> => {
      ...
      try {
        const s: any = await api.getOpenShift();
        if (s) {
  ...
  const value: Ctx = {
    ...
    openShift: async (cash) => {
      try {
        const s: any = await api.openShift({ opening_float: Number(cash) || 0 });
        setShift({ open: true, id: s.id, openedAt: new Date(s.opened_at).getTime(), openingCash: Number(s.opening_float) });
        await refreshHeld(s.id);
        setScreen("pos");
      } catch (e: any) {
    ...
    resumeHeld: async (id) => {
      try {
        const row: any = await api.resumeHeldOrder({ id });
    ...
    completeOrder: async (payment) => {
      ...
      try {
        ...
        if (customerPhone?.trim()) {
          try {
            const c: any = await api.findOrCreateCustomerByPhone({ phone: customerPhone.trim() });
            customer_id = c?.id;
          } catch (err: any) {
